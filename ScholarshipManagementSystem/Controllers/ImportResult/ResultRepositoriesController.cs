...
namespace ScholarshipManagementSystem.Controllers.MasterSetup
{
    public class ResultRepositoriesController : Controller
    {
        ...
        public async Task<JsonResult> UploadFilePost(IFormFile excelFile, List<string> selectedColumn, int PolicySRCForumId, int SchemeId, int SchemeLevelId, int DegreeScholarshipLevelId)
        {
            if (excelFile != null)
            {
                ...
                if (SchemeId < 4)
                {
                    IsResultAlreadyExist = _context.ResultRepository.Include(a => a.SchemeLevelPolicy).Count(a => a.SchemeLevelPolicy.PolicySRCForumId == PolicySRCForumId && a.SchemeLevelPolicy.SchemeLevelId == SchemeLevelId);
                }
                else
                {
                    IsResultAlreadyExist = _context.ResultRepository.Include(a => a.SchemeLevelPolicy).Count(a => a.ScholarshipFiscalYearId == PolicySRCForumId && a.DegreeScholarshipLevelId == DegreeScholarshipLevelId);
                }
                if(IsResultAlreadyExist == 0)
                {
                    ...
                    resultRepository.IsDataCleaned = false;
                    ...
                    return Json(new { isValid = true, message = "Result Imported successfully!", RRId = MaxResultRespositoryId });
                }
                else
                {
                    return Json(new { isValid = false, message = "Result already exist!" });
                }
            }
            else
            {
                return Json(new { isValid = false , message = "File not attached!" });
            }
       ...
       /* [HttpPost]
        ...
        public async Task<int> CallSP(string SP,string column, int rrId, int DocumentAssistId)
        {
            var applicationDbContext = _context.SPAssistDocumentViewer.FromSqlRaw<SPAssistDocumentViewer>(SP, column, rrId);
            int result = 0;
            try
            {
                if (DocumentAssistId == 1)
                {
                    var temp = applicationDbContext.ToList();
                    result = temp.Count();
                }
        ...
        public async Task<IActionResult> AssistDocument(int rrId)
        {
            ...
            foreach (var record in records)
            {
                if (record.DocumentAssistId == 1)
                {
                    ...
                    obj.TotalFind = await CallSP("exec [ImportResult].[SPCheckDouplicate] {0}, {1}", record.ExcelColumnName.Name, rrId, record.DocumentAssistId);
                    _context.Update(obj);
                }
                else if (record.DocumentAssistId == 2)
                {
                    DocumentAssistIndicator obj = new DocumentAssistIndicator();
                    obj = _context.DocumentAssistIndicator.Find(record.DocumentAssistIndicatorId);
                    obj.TotalFind = await CallSP("exec [ImportResult].[SPCheckNullValues] {0}, {1}", record.ExcelColumnName.Name, rrId, record.DocumentAssistId);
                    _context.Update(obj);
                }
            }

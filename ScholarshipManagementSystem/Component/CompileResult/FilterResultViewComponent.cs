...
namespace ScholarshipManagementSystem.Component.CompileResult
{
    public class FilterResultViewComponent : ViewComponent
    {
        ...
        {
            int GradingSystem = await _context.ResultRepository.Include(a => a.SchemeLevelPolicy.SchemeLevel).Where(a => a.ResultRepositoryId == RRId).Select(a => a.SchemeLevelPolicy.SchemeLevel.GradingSystem).FirstOrDefaultAsync();            
